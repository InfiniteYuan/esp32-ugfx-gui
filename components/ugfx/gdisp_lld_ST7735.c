...
{
    write_cmddata(g, ST7735_CASET, MAKEWORD((g->p.x) >> 8, (g->p.x) & 0xFF, (g->p.x+g->p.cx-1) >> 8, (g->p.x+g->p.cx-1) & 0xFF));
    write_cmddata(g, ST7735_RASET, MAKEWORD((g->p.y) >> 8, (g->p.y) & 0xFF, (g->p.y+g->p.cy-1) >> 8, (g->p.y+g->p.cy-1) & 0xFF));
...
LLDSPEC void gdisp_lld_fill_area(GDisplay *g)
{
	...
	acquire_bus(g);
	set_viewport (g);
	write_data_byte_repeat (g,c,g->p.cx*g->p.cy);
	release_bus(g);
}
